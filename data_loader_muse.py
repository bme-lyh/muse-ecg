        ...
        if self.y is None:
            ...
            if self.transform:
                for transform_func in self.transform:
                    if type(x_sample) == tuple:
                        x_sample, y_sample = transform_func(x_sample[0], x_sample[1])
                    else:
                        x_sample = transform_func(x_sample)
            
            x_sample = torch.from_numpy(x_sample.astype(np.float32)).permute(1, 0).unsqueeze(1)
            if y_sample is not None:
                y_sample = torch.from_numpy(y_sample.astype(np.float32))
...
def raw_data_load_rdb(num_test, num_labeled, fold, crop=[1250, 3750], apply_flag=True):
    ...
    if num_test == 0:
        return x_rdb[flag_rdb==0], y_rdb[flag_rdb==0]
...
def get_strong_transforms():
    transforms = [
        lambda x: x + additive_white_gaussian_noise(x[:,0], snr=10)[:,np.newaxis],
        lambda x: x + baseline_wander_noise(x[:,0], fs=500, snr=-10, freq=0.15)[:,np.newaxis]  if np.random.random() < 0.5 else x,
        lambda x: zscore_normalize(x, axis=0)
    ]
    return transforms
...
def get_weak_transforms():
    transforms = [
        lambda x: x + additive_white_gaussian_noise(x[:,0], snr=20)[:,np.newaxis],
        lambda x: zscore_normalize(x, axis=0)
    ]
    return transforms


def get_self_supervised_transforms():
    transforms = [
        lambda x: x + additive_white_gaussian_noise(x[:,0], snr=10)[:,np.newaxis],
        lambda x: x + baseline_wander_noise(x[:,0], fs=500, snr=-10, freq=0.15)[:,np.newaxis]  if np.random.random() < 0.5 else x,
        lambda x: rand_mask(x, mask_ratio=0.8*np.random.random()),
        lambda x: (np.flip(x, axis=0), np.array([1])) if np.random.random() < 0.5 else (x, np.array([0])),
        lambda x, y: (zscore_normalize(x, axis=0), y),
    ]
    return transforms


def base_transforms(label=True):
    if label:
        transforms = [
            lambda x, y: (zscore_normalize(x, axis=0), y),
        ]
